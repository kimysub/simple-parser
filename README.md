...
- **PPTX**: Slide titles, body text, tables, correct slide ordering
- **XLSX**: Sheet names, shared strings, numeric values → markdown tables
- **PDF**: Text extraction via PyMuPDF, table detection, font-size based heading detection (⚠️ math equations may render incorrectly)
...
PDF parsing uses PyMuPDF (`fitz`) for non-OCR text extraction with table detection (`page.find_tables()`) and a font-size heuristic for heading detection. Note: mathematical equations in PDFs may not render correctly — PDFs store equations as positioned glyphs rather than semantic math notation, so spatial constructs (summations, fractions, sub/superscripts) get fragmented during text extraction.
